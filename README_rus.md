...
│   ├── player.py              # Воспроизведение (юзается pygame.mixer)
...
│   │   └── Roboto-Bold.ttf
│   └── bin/
│       ├── ffmpeg.exe         # Опциональная штука, для поддержка m4a и прочего
│       └── ffprobe.exe        # Опционально, можно юзать в приложениях например.
│
