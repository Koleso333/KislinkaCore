...
Фреймворк для приложений сделанный на Python + PyQt6.

KislinkaCore это что то типа движка для приложений для пк. Типа ты пишешь логику приложения, а само приложение это выполняет и рисует в окошке. Есть темы, аудио, графика, локализация, сохранение данных и т.д..

Документация создана для версии 0.1.1b1 (и коммитов)
---
...
- [Создание приложений](#creating-an-application)
  - [manifest.json](#manifestjson)
  - [Главный класс](#main-class)
  - [Цикл жизни](#lifecycle)
- [API юзабилити](#api-reference)
  ...
  - [Компоненты](#components)
    - [Как создать компонент](#creating-a-component)
    - [Хуки](#hooks)
    - [Доступ к компонентам из приложений](#accessing-components-from-apps)
  - [Тайтлбар](#titlebar)
  - [Вкладки настроек](#settings-tabs)
  - [Шрифты](#fonts)
- [Тематизация](#theming)
- [Обработчик ошибок](#error-handling)
- [Лаунчер и Splash](#launcher--splash)
- [Путь файлов и ассетов](#file-paths--assets)
...
Качай [Mitr Regular](https://fonts.google.com/specimen/Mitr) и [Roboto Bold](https://fonts.google.com/specimen/Roboto) из Google Fonts, но можешь и свои.
...
class MyApp:
    def setup(self, app):
        ...
        lay.addWidget(KLabel("Здарова", style="heading",
                              align=Qt.AlignmentFlag.AlignCenter))
        lay.addWidget(KButton("Нажми на меня", on_click=self.on_click))
...
KislinkaCore/
│
├── main.py                    # Главный вход в ТЦ
├── requirements.txt
│
├── core/                      # Тут движок
│   ├── app.py                 # KislinkaApp - Главный контроллер
│   ├── window.py              # Окошко
│   ├── titlebar.py            # Тут тайтлбар
│   ├── scene.py               # Менеджер сцен и анимаций для них
│   ├── theme.py               # Система темной и светлой темы.
│   ├── fonts.py               # Загрузчик шрифтов
│   ├── permissions.py         # Core / App / User права.
│   ├── settings.py            # Встроенная вкладка настроек.
│   ├── loader.py              # Загрузчик и сканер приложений. Есть они ил нет.
│   ├── storage.py             # Хранитель данных типа (JSON в AppData)
│   ├── locale.py              # Локализация (важно: у ядра и приложения свои локализации, они независемы.)
│   ├── hooks.py               # Система хуков для компонентов
│   ├── component.py           # Базовый класс для компонентов
│   ├── component_manager.py   # Сканер компонентов и загрузчик
│   ├── error_handler.py       # Глобальный обработчик ошибок и плюсом окошко ошибки.
│   ├── launcher.py            # Лаунчер если приложений несколько
│   ├── splash.py              # Splash анимация
│   ├── core_info.json         # Метаданный ядра (версия и т.д.)
│   └── locales/
│       ├── en.json            # Английский перевод
│       └── ru.json            # Русский перевод
│
├── components/                # Тут компоненты будут лежать.
│   └── MyComponent/           # Пример названия
│       ├── manifest.json      # Пример
│       └── component.py       # Пример
│
├── widgets/                   # UI компоненты
│   ├── kbutton.py             # Кнопочка стандартная
│   ├── klabel.py              # Label
│   ├── ktextfield.py          # Текстовое окно (многострочное или однострочное)
│   ├── ktoggle.py             # Переключатель
│   ├── kslider.py             # Слайдер 
│   ├── kgrid.py               # KRow, KColumn, KGrid лайаут компановка
│   ├── kpanel.py              # Панель для контейнеров
│   ├── kdivider.py            # Панель для разделения
│   ├── kscrollarea.py         # scroll area
│   ├── kicon.py               # Загрузчик SVG иконок
│   └── ksettingsitem.py       # Список натроек
│
├── audio/                     # Система для аудио
│   ├── player.py              # Воспроизведение (PyAV + sounddevice)
│   └── metadata.py            # Для метаданных аудио (юзается mutagen)
│
├── graphics/                  # Шоб рисовать
│   ├── canvas.py              # Виджет где и будет рисоваться
│   ├── shapes.py              # Примитивы (rect, circle, и т.д.)
│   ├── image.py               # Загрузка / изменение изображений
│   └── effects.py             # Цвет, прозрачность, инвертирование, закруглённые углы.
│
├── assets/
│   ├── fonts/
│   │   ├── Mitr-Regular.ttf
│   │   └── Roboto-Bold.ttf
│   └── bin/                   # Опциональные бинарники
│
└── App/                       # Тута приложения лежат
    └── MyApp/
        ├── manifest.json
        ├── main.py
        ├── assets/            # Специфичные ассеты для приложения (опционально)
        └── locales/           # Перевод приложений (опционально, но желательно)
            ├── en.json
            └── ru.json
```
...
1. `main.py` создаёт `KislinkaApp`
2. Ядро сканирует `components/` и загружает компоненты
3. Ядро сканирует `App/` для загрузки приложений
4. Если приложение одно → запускает приложение вместе с Splash
5. Если приложений 2 и более → показывает лаунчер и даёт пользователю выбор кого запускать
6. Если вообще 0 приложений → показывает окно ошибки
7. Ядро создаёт `KislinkaWindow`, `SceneManager`, `SettingsPanel`
8. Ядро вызывает `YourApp.setup(app)` - и тут твои сцены
9. На выходе, ядро вызывает `YourApp.cleanup()`
...
| Поле            | Обязательно ли      | Описание                                           |
|-----------------|---------------------|----------------------------------------------------|
| `name`          | Да                  | Уникальное имя приложения (без пробелов)           |
| `display_name`  | Да                  | Описание                                           |
| `version`       | Да                  | Версия приложения                                  |
| `author`        | Да                  | Автор.                                             |
| `main_class`    | Да                  | Имя класса в entry_point файле                     |
| `entry_point`   | Нет                 | Физический файл что запускается, дэфолт: `main.py` |
| `window.width`  | Нет                 | Ширина окна, дэфолт: 900                           |
| `window.height` | Нет                 | Высота окна, дэфолт: 600                           |
| `permissions`   | Нет (но желательно) | Флаги функций.                                     |
...
class MyApp:
    def setup(self, app):
        """
        Вызывается ядром после загрузки

        app: KislinkaApp имеет доступ к:
    ...
    def cleanup(self):
        ...
        Остановка аудио, сохранение данных и т.д.
        """
        pass
```
...
```
1. Ядро вызывает  setup(app)
   → Сборка сцены, подключение сигналов, загрузка данных

2. Приложение запускается - пользователь юзает приложение (сцены и т.д.)

3. Ядро вызывает cleanup()
   → Stop audio, save state, disconnect signals
   → Остановка аудио, сохранение данных, отключение сигналов и т.д.
   (Также вызывается при смене языка перед перезагрузкой)
```
...
Доступ в `setup(self, app)`:

```python
app.scene_manager    # SceneManager — открытие / закрытие сцен
app.theme_manager    # ThemeManager — смена темы
app.audio            # AudioPlayer  — play music
app.storage          # StorageManager — сохранение и загрузка данных
app.locale           # LocaleManager — перевод строк
app.permissions      # PermissionManager — регистрация вкладок настроек
app.window           # KislinkaWindow — доступ к строке заголовка, размеры окна
app.hooks            # HookManager — регистрация / выдача хуков
app.components       # ComponentManager — доступ к загруженным компонентам
```
...
Сцены отображаются в окне. нажимайте, перемещайте и заменяйте с анимацией.
...
sm.push(scene, AnimationType.SLIDE_LEFT)     # Анимация улетания в →
sm.push(scene, AnimationType.SLIDE_RIGHT)    # Анимация улетания в ←
sm.push(scene, AnimationType.NONE)           # Без анимации

sm.pop(AnimationType.SLIDE_RIGHT)            # Назад 
sm.replace(scene, AnimationType.SLIDE_LEFT)  # замена текущего

# Свойства
sm.current          # Текущая сцена или её нет
sm.stack_depth      # инт
sm.is_animating     # Проверка перед нажатием
```
...
player.play("track.m4a")           # M4A/AAC поддерживается нативно
...
Все форматы работают из коробки — никаких внешних бинарников не нужно.
Пакет `av` (pip) содержит FFmpeg библиотеки внутри.
...
Внешние бинарники не нужны — все аудиокодеки уже внутри pip-пакета `av`.
